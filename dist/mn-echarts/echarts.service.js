...
var EchartsService = (function () {
    ...
    EchartsService.prototype.getEchartResult = function (type, data, setting, $charts, $mycharts) {
        ...
        fn.sort = function () {
            // 排序
            mu.run(setting.module === 'single', function () {
                mu.run(setting.sort, function (sortInfo) {
                    sortInfo = mu.type(sortInfo, 'boolean') ? Y_VALUE : sortInfo;
                    var sortInfo_ = sortInfo.split(':');
                    var key = sortInfo_[0] || Y_VALUE;
                    var sort = sortInfo_[1] || 'desc';
        ...
        fn.$xAxis = function () {
            ...
            mu.run(setting.module, function (module) {
                switch (module) {
                    ...
                    case 'group':
                    case 'mix':
                        _x_axis = options.xAxis[0].data = mu.map(mu.groupArray(data, X_VALUE), function (o, x) { return x; }, []);
                        break;
                }
            });
        };
        ...
        fn.xAxis_rotate = function () {
            mu.run(setting.rotate, function (rotate) {
                options.xAxis[0].axisLabel.interval = 0;
