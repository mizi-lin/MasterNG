        ...
        this.getShow = mu.debounce(function () {
            mu.run(_this._show, function () {
                mu.run(_this.maxDate_, function (max) {
                    // 获取下一年这月的时间戳
                    var next_year = _this._view.yoy();
                    _this._show.next_year = next_year.year_range.start < max.current;
                    _this._show.next_month = _this._view.month_range.end < max.current;
                });
                mu.run(_this.minDate_, function (min) {
                    var prev_year = _this._view.yoy(-1);
                    _this._show.prev_year = prev_year.year_range.end > min.current;
                    _this._show.prev_month = _this._view.month_range.start > min.current;
                });
            });
        }, 300);
    ...
    Object.defineProperty(MnCalendarComponent.prototype, "viewComponent", {
        set: function (_vc) {
            ...
            _vc.selectedDate = function (date) {
                if (_this.mode === 'single' && !date.not_selected) {
